...
%%{init: {"flowchart": {"curve": "stepAfter"}}}%%
flowchart LR
  K["Available K"] --> KM["Deliver to a<br/>microbiome"] --> KP["20 per loose K<br/>max 3 per microbiome"]
  K --> KH["Deliver to<br/>central haven"] --> KHP["5 per K fully inside<br/>shared by both teams"]
  R["Available R"] --> RB["Leave in<br/>grand-tree base"] --> RBP["5 per R<br/>remaining in base"]
  R --> RC["Cycle from base<br/>up to canopy"] --> RCP["10 per R in canopy<br/>max 15 R; not 5 + 10"]
...
%%{init: {"flowchart": {"curve": "stepAfter"}}}%%
flowchart LR
  READY["Have a K AND keep<br/>tree holding lid open"] --> PASS["Feed K through waterfall<br/>5 per cycle; max 50"]
  PASS --> EXIT["K exits at bottom<br/>beside score meter"]
  EXIT -->|"collect with driving base"| RECOVER["Recovered K"]
  RECOVER -->|"re-feed while lid stays open"| PASS
```
...
%%{init: {"flowchart": {"curve": "stepAfter"}}}%%
flowchart LR
  FOUR["Fourth K delivered<br/>to our young forest"] --> QUEEN["Opposite queen<br/>knocked down"]
  QUEEN --> BONUS["40-point bonus IF at least<br/>3 K remain in our forest"]
  QUEEN --> CONTAIN["10 containment points IF<br/>queen ends in containment"]
```
